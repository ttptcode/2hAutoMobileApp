...
public class CreateBatteryPostActivity extends AppCompatActivity {
    ...
    private void loadExistingListingData(Listing listing) {
        ...
        String batteryType = getItemStringField(item, "getBatteryType");
        if (batteryType != null) setSpinnerValue(spinnerType, batteryType);
        String voltage = getItemStringField(item, "getVoltage");
        if (voltage != null) setSpinnerValue(spinnerVoltage, voltage);
        String capacity = getItemStringField(item, "getCapacity");
        if (capacity != null) setSpinnerValue(spinnerCapacity, capacity);
