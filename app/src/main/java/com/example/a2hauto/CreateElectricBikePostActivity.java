...
public class CreateElectricBikePostActivity extends AppCompatActivity {
    ...
    private void submitPost(boolean isDraft) {
        ...
        String vehicleTypeValue = spinnerVehicleType.getText().toString().trim();
        fields.put("VehicleType", createPartFromString(vehicleTypeValue.isEmpty() ? "Xe đạp điện" : vehicleTypeValue));
        
        String motorValue = spinnerMotor.getText().toString().trim();
        fields.put("MotorPower", createPartFromString(motorValue.isEmpty() ? "200 - 250W" : motorValue));
        
        String batteryValue = etBatteryCapacity.getText().toString().trim();
        fields.put("BatteryCapacity", createPartFromString(batteryValue.isEmpty() ? "0" : batteryValue));
        ...
        fields.put("Origin", createPartFromString(originValue.isEmpty() ? "Trung Quốc" : originValue));
        
        String colorValue = etColor.getText().toString().trim();
        ...
        fields.put("Warranty", createPartFromString(warrantyValue.isEmpty() ? "Hết bảo hành" : warrantyValue));
